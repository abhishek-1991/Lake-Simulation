...
void evolve9pt(double *un, double *uc, double *uo, double *pebbles, int n, double h, double dt, double t);	//function for 9pt stencil
...
int main(int argc, char *argv[])
{
  ...
  //checking for appropriate parameters
  if(argc <4 || argc > 5)
  {
...
void run_cpu(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  while(1)
  {
   //calling function for 9pt stencil
...
void evolve9pt(double *un, double *uc, double *uo, double *pebbles, int n, double h, double dt, double t)
{
  ...
  for( i = 0; i < n; i++)
  {
    for( j = 0; j < n; j++)
    {
      ...
      {
	//formula for 9pt stencil calculation
        un[idx] = 2*uc[idx] - uo[idx] + VSQR *(dt * dt) *((uc[idx-1] + uc[idx+1] + 
